...
TEST_F(ExporterTest, exportCOETest) {
...
TEST_F(ExporterTest, exportHexTest) {
    ...
    EXPECT_EQ(exportedHex, hexString);
}

TEST_F(ExporterTest, exportASCIIBinTest) {
    ...
    EXPECT_EQ(exportedBin, binString);
}

TEST_F(ExporterTest, exportBinTest) {
    ...
    for (int i = 0; i < 24; ++i) {
        EXPECT_EQ(bin[i], exportedUnsignedBin[i]);
    }
}
